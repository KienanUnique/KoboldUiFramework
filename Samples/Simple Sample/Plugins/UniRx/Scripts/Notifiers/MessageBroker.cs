...
namespace SampleUnirx
{
    ...
    public class AsyncMessageBroker : IAsyncMessageBroker, IDisposable
    {
        ...
        {
            SampleUnirx.InternalUtil.ImmutableList<Func<T, IObservable<Unit>>> notifier;
            lock (notifiers)
            {
                ...
                {
                    notifier = (SampleUnirx.InternalUtil.ImmutableList<Func<T, IObservable<Unit>>>)_notifier;
                }
                else
                {
        ...
        public IDisposable Subscribe<T>(Func<T, IObservable<Unit>> asyncMessageReceiver)
        {
            lock (notifiers)
            {
                ...
                if (!notifiers.TryGetValue(typeof(T), out _notifier))
                {
                    var notifier = SampleUnirx.InternalUtil.ImmutableList<Func<T, IObservable<Unit>>>.Empty;
